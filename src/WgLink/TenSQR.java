...
class ThreadF implements Runnable {
	...
	public ThreadF(String [] path, String region) {
...
//		System.out.println(r+"\t"+log);
		if (TenSQR.log_dic.containsKey(r)) {
				TenSQR.log_dic.put(r, TenSQR.log_dic.get(r)+"\n"+log);
			}else {
				TenSQR.log_dic.put(r, log);
		}
	}
...
//			br.close();
			System.out.println("Region\t"+ this.region + " TenSQR finished.");
			Thread.sleep( 10);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
public class TenSQR {
	...
	public TenSQR(String [] cmds, String [] regions) throws IOException {
...
//			System.out.println(this.cmdArray[i] );
			...
			exec.execute(new ThreadF(tmp, this.regionArray[i]));
		}
...
//						System.out.println("????\t"+key);
						if ((!this.log_dic.get(key).equals(null))  &&  (!this.log_dic.get(key).equals(""))) {
							System.out.println("TenSQR Prompt the Message on Region "+key+":");
